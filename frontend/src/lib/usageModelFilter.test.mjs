...
test('Usage model filter keeps Antigravity and Claude catalogs available without logs', () => {
  ...
  const seenModels = extractBalancedBody(usageSource, 'const seenModels = useMemo(() =>')
  assert.equal(seenModels.includes('for (const item of modelStats)'), true)
  assert.equal(modelFilterOptions.includes('for (const key of seenModels)'), true)
