...
function getInitialUsageRange(): UsageTimeRangeKey {
  ...
  if (USAGE_TIME_RANGE_OPTIONS.includes(range as UsagePresetRangeKey)) {
    return range as UsagePresetRangeKey
  }
  return '24h'
}
...
export default function Usage() {
  ...
  const statsTimeRangeRef = useRef(timeRange)
  statsTimeRangeRef.current = timeRange

  const loadStats = useCallback(async () => {
    const todayRange = getTodayRangeISO()
    const filterParams = statsFilterParamsRef.current()
    const currentTimeRange = statsTimeRangeRef.current
    const [stats, settings] = await Promise.all([
      api.getUsageStats(filterParams),
      api.getSettings().catch((): SystemSettings | null => null),
    ])
    const todayStats = currentTimeRange === '1h'
      ? await api.getUsageStats({
          ...filterParams,
          start: todayRange.start,
          end: todayRange.end,
        }).catch((): UsageStats | null => null)
      : null
    return { stats, settings, todayStats }
  }, [])
  ...
  const { data, loading, error, reload, reloadSilently } = useDataLoader<{
    ...
    initialData: { stats: null, settings: null, todayStats: null },
    load: loadStats,
  })
  ...
  const { stats, settings, todayStats } = data
  ...
  const modelFilterOptions = useMemo(() => {
    ...
    return merged
  }, [modelOptions, grokModelOptions, antigravityModelOptions, claudeModelOptions, seenModels, channel, filterModel])
  ...
  const todayRequests = todayStats?.today_requests ?? (timeRange === 'today' ? rangeRequests : 0)
