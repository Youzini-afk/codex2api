...
func (h *Handler) mergeRefreshedDuplicateIntoExistingContext(parent context.Context, newID int64, source string) bool {
	...
	for _, key := range []string{"refresh_token", "session_token", "access_token", "access_token_type", "id_token", "expires_at", "email", "account_id", "chatgpt_account_id", "workspace_id", "user_id", "chatgpt_user_id", "plan_type", "subscription_expires_at"} {
...
type accountSchedulerUpdate struct {
	...
	UsageReservePercent5h   database.OptionalNullInt64
	UsageReservePercent7d   database.OptionalNullInt64
...
func parseAccountSchedulerUpdate(req updateAccountSchedulerReq) (accountSchedulerUpdate, error) {
	...
	return accountSchedulerUpdate{
		...
		UsageReservePercent5h:   usageReserve5h,
		UsageReservePercent7d:   usageReserve7d,
...
func (u accountSchedulerUpdate) hasChanges() bool {
	return u.ScoreBiasOverride.Set ||
		...
		u.UsageReservePercent5h.Set ||
		u.UsageReservePercent7d.Set ||
		u.SkipWarmTier.Set ||
...
func (h *Handler) UpdateAccountScheduler(c *gin.Context) {
	...
	if err := h.db.UpdateAccountSchedulerMetadata(ctx, id, update.ScoreBiasOverride, update.BaseConcurrencyOverride, update.UsageReservePercent5h, update.UsageReservePercent7d, update.SkipWarmTier, update.AllowedAPIKeyIDs, database.OptionalStringSlice{Set: update.Tags.Set, Values: update.Tags.Values}, update.GroupIDs, update.ProxyURL, update.CredentialUpdates); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			writeError(c, http.StatusNotFound, "账号不存在")
			return
		}
...
func (h *Handler) applyAccountSchedulerRuntimeUpdate(id int64, update accountSchedulerUpdate) {
	...
	if update.UsageReservePercent5h.Set || update.UsageReservePercent7d.Set {
		var reserve5h *int64
		var reserve7d *int64
		if current := h.store.FindByID(id); current != nil {
			if value, ok := current.GetUsageReservePercent5h(); ok {
				reserve5h = &value
			}
			if value, ok := current.GetUsageReservePercent7d(); ok {
				reserve7d = &value
			}
		}
		if update.UsageReservePercent5h.Set {
			reserve5h = nullableInt64Pointer(update.UsageReservePercent5h.Value)
		}
		if update.UsageReservePercent7d.Set {
			reserve7d = nullableInt64Pointer(update.UsageReservePercent7d.Value)
		}
		h.store.ApplyAccountUsageReserveOverrides(id, reserve5h, reserve7d)
	}
...
func (h *Handler) PushTokenAccount(c *gin.Context) {
	...
	if req.RefreshToken != "" {
		...
		if credentials := tokenCredentialMap(seed); len(credentials) > 0 {
		...
		if h.refreshAccount != nil {
			h.startDBBackgroundTask(func(ctx context.Context) {
				h.refreshImportedAccountAndProbe(ctx, id, "push_token")
			})
		}
	...
	var tokens []importToken
	{
		...
		if rt != "" || st != "" || at != "" {
			tokens = append(tokens, importToken{
				...
				userID:                firstNonEmpty(c.ChatGPTUserID, c.UserID, c.ChatGPTAccountUserID, c.AccountUserID),
				planType:              planType,
				expiresAt:             expiresAt,
...
func importTokenAccountScopeForImport(t importToken, conflicts map[string]bool) string {
	if accountID := strings.TrimSpace(t.accountID); accountID != "" {
		return accountID
	}
	if conflicts == nil {
		return strings.TrimSpace(t.chatgptAccountID)
	}
	return reliableImportChatGPTID(t, conflicts)
}

func importTokenAccountScopeForDedupe(t importToken, conflicts map[string]bool) string {
	if accountID := strings.TrimSpace(t.accountID); accountID != "" {
		return accountID
	}
	id := strings.TrimSpace(t.chatgptAccountID)
	...
	if conflicts[id] && strings.TrimSpace(t.accessToken) != "" {
		return ""
	}
	return id
}
...
func importTokenDBUserIdentityKey(t importToken) string {
	return importTokenDBScopedIdentity(importTokenAccountScopeForImport(t, nil), t.userID)
}

func importTokenDBEmailIdentityKey(t importToken) string {
	return importTokenDBScopedIdentity(importTokenAccountScopeForImport(t, nil), strings.ToLower(strings.TrimSpace(t.email)))
}
...
func importTokenChatGPTDedupeKey(t importToken) string {
	scope := importTokenAccountScope(t)
...
func (h *Handler) importAccountsCommon(c *gin.Context, tokens []importToken, proxyURL string, allowDuplicate bool, customHeaders ...map[string]string) {
	...
	// 2) 当条目带有 chatgpt_account_id/account_id 但没有 OAuth 身份时，优先以 (workspace/account, user_id/email) 区分，
	...
	for _, t := range tokens {
		...
		if importTokenAccountScopeForDedupe(t, conflictingChatGPTIDs) != "" {
	...
	if allowDuplicate && !workspaceOverrideKnown {
		...
		for _, t := range unique {
			seed := importTokenSeed(t, conflictingChatGPTIDs)
			seed.customHeaders = cloneCustomHeaders(importCustomHeaders)
			if effectiveWorkspaceIDFromSeed(seed) == "" && importTokenAccountScopeForDedupe(t, conflictingChatGPTIDs) != "" && importTokenHasFineIdentity(t) {
		...
		for _, t := range unique {
			...
			seed.customHeaders = cloneCustomHeaders(importCustomHeaders)
			if seed.email != "" && effectiveWorkspaceIDFromSeed(seed) != "" {
				duplicateID, identityErr := h.findOAuthIdentityDuplicate(dedupeCtx, seed, 0)
				if identityErr != nil {
					log.Printf("查询已有 OAuth 身份失败: %v", identityErr)
				} else if duplicateID > 0 {
					row, err := h.db.GetAccountByID(dedupeCtx, duplicateID)
					if err != nil {
						log.Printf("查询已有 OAuth 账号 %d 失败: %v", duplicateID, err)
					...
					// Same effective identity with rotated credentials is not a new
					// account: keep it in the work set so the upsert path updates the
			...
			// Legacy exports may carry account/user identity but no token-derived
			// workspace. Keep the fork's rotation-aware identity checks for those
			// rows. Once an effective route is known, route-aware OAuth identity
			// above takes precedence so a personal route cannot hide a team route.
			if effectiveWorkspaceIDFromSeed(seed) == "" {
				if importTokenDBIdentityExists(t, existingIdentities) {
					duplicateCount++
					continue
				}
				// Only entries without a finer user/email identity fall back to the
				// historical workspace/account identifier.
				if scope := importTokenAccountScopeForDedupe(t, conflictingChatGPTIDs); scope != "" && !importTokenHasFineIdentity(t) && existingChatGPTIDs[scope] {
					duplicateCount++
					continue
				}
			}
...
func (h *Handler) UpdateSettings(c *gin.Context) {
	...
	if existingSettings != nil {
		...
		previousAdminSecret = existingSettings.AdminSecret
