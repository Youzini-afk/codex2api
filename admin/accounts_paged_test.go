...
func TestPagedAccountRuntimeStatusOverridesDatabaseRow(t *testing.T) {
	handler, codexIDs, _ := newPagedAccountsHandler(t)
	// This test intentionally creates an in-memory state newer than the database.
	// Stop the outbox consumer first so it cannot replay the earlier insert event
	// and overwrite that synthetic runtime state while the snapshot is built.
	handler.store.Stop()
