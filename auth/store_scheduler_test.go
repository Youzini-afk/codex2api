...
func TestLegacyUsageReserveDoesNotAffectUpstreamAvailability(t *testing.T) {
	now := time.Now()
	acc := &Account{
		AccessToken:           "token",
		Status:                StatusReady,
	...
	if !acc.IsAvailable() {
		t.Fatal("legacy usage reserve unexpectedly blocked current upstream availability")
	}
	if got := acc.RuntimeStatus(); got != "active" {
		t.Fatalf("RuntimeStatus() = %q, want active with retired reserve metadata", got)
	}
}
