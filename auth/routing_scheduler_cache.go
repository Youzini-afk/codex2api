...
// routingSchedulerTouchSeq provides a unique, monotonic order for cache
// accesses. A wall-clock timestamp is not suitable for LRU ordering because
// adjacent accesses can share the same clock tick on some platforms.
var routingSchedulerTouchSeq atomic.Uint64

func touchRoutingSchedulerEntry(entry *routingSchedulerEntry) {
	seq := routingSchedulerTouchSeq.Add(1)
	for {
		previous := entry.lastHitSeq.Load()
		if previous >= seq || entry.lastHitSeq.CompareAndSwap(previous, seq) {
			return
		}
	}
}
...
func (s *Store) routingFastScheduler(apiKeyID int64) *FastScheduler {
	...
	if ok {
		touchRoutingSchedulerEntry(entry)
	}
...
func (s *Store) evictRoutingSchedulerLocked(alias bool) bool {
	...
	for key, entry := range s.routingSchedulers {
		...
		if victim == nil || entry.lastHitSeq.Load() < victim.lastHitSeq.Load() {
			victimKey = key
			victim = entry
		}
	}
