...
func TestRoutingSchedulerEvictsUntouchedEntryAfterRecentHit(t *testing.T) {
	accounts := sparseRoutingAccounts(16, 9)
	store := newIndexedRoutingTestStore(accounts)
	// Configure all keys first because changing routing rules invalidates the
	// cache. The final key is then inserted after key 1 is touched again.
	for key := int64(1); key <= int64(maxRoutingSchedulerKeys)+1; key++ {
		store.SetAPIKeyAllowedGroups(key, []int64{9})
	}
	for key := int64(1); key <= int64(maxRoutingSchedulerKeys); key++ {
		if acc := store.NextExcluding(key, nil); acc != nil {
			store.Release(acc)
		}
	}

	if acc := store.NextExcluding(1, nil); acc == nil {
		t.Fatal("recent hit returned nil")
	} else {
		store.Release(acc)
	}
	if acc := store.NextExcluding(int64(maxRoutingSchedulerKeys)+1, nil); acc == nil {
		t.Fatal("new key selection returned nil")
	} else {
		store.Release(acc)
	}

	store.routingSchedulersMu.RLock()
	_, touched := store.routingSchedulers[1]
	_, untouched := store.routingSchedulers[2]
	_, newest := store.routingSchedulers[int64(maxRoutingSchedulerKeys)+1]
	store.routingSchedulersMu.RUnlock()
	if !touched {
		t.Fatal("recently hit key was evicted")
	}
	if untouched {
		t.Fatal("least recently used untouched key was retained")
	}
	if !newest {
		t.Fatal("newest key missing from cache")
	}
}

func TestRoutingSchedulerCacheConcurrentAccess(t *testing.T) {
	store := newIndexedRoutingTestStore(sparseRoutingAccounts(16, 9))
	const keyCount = maxRoutingSchedulerKeys + 32
	for key := int64(1); key <= keyCount; key++ {
		store.SetAPIKeyAllowedGroups(key, []int64{9})
	}
	...
	for worker := 0; worker < workers; worker++ {
		go func(worker int) {
			for round := 0; round < rounds; round++ {
				for offset := worker; offset < keyCount; offset += workers {
					if store.routingFastScheduler(int64(offset+1)) == nil {
						t.Errorf("worker %d got nil scheduler for key %d", worker, offset+1)
					}
				}
			}
			done <- struct{}{}
	...
	store.routingSchedulersMu.RLock()
	entries := len(store.routingSchedulers)
	accounts := store.routingSchedulerAccounts
	store.routingSchedulersMu.RUnlock()
	if entries > maxRoutingSchedulerKeys {
		t.Fatalf("concurrent cache entries = %d, want at most %d", entries, maxRoutingSchedulerKeys)
	}
	if accounts < 0 || accounts > maxRoutingSchedulerAccounts {
		t.Fatalf("concurrent cached accounts = %d, want within [0,%d]", accounts, maxRoutingSchedulerAccounts)
	}
}
