...
// 全局优先级必须先于历史验证状态：低优先级 proven 账号不能越过高优先级账号。
func TestFastSchedulerPriorityPrecedesProvenHistory(t *testing.T) {
	lowProven := newFastSchedulerTestAccount(1, HealthTierHealthy, 120, 1)
	lowProven.TotalRequests = 11
	highUnproven := newFastSchedulerTestAccount(2, HealthTierHealthy, 80, 1)
	highUnproven.SetSchedulerPriority(10)

	scheduler := NewFastScheduler(1, "round_robin")
	scheduler.Rebuild([]*Account{lowProven, highUnproven})
	...
	if got.DBID != highUnproven.DBID {
		t.Fatalf("Acquire() picked dbID=%d, want high-priority unproven account %d", got.DBID, highUnproven.DBID)
	}
}
