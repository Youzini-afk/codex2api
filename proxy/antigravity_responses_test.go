...
func TestAntigravityInteractionsRequestEndpointHeadersAndBody(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = io.WriteString(w, `{"id":"int_test","object":"interaction","status":"completed","steps":[],"usage":{"total_tokens":0}}`)
	}))
	defer server.Close()
...
func TestAntigravityInteractionsMalformedSuccessIsRetryableBadGateway(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	account := &auth.Account{DBID: 57, UpstreamType: auth.UpstreamAntigravity, APIKey: "google-key"}
	_, err := executeAntigravityInteractionsRequest(context.Background(), account, "gemini-2.5-flash", []byte(`{"input":"hello"}`), false, "")
	var apiErr *Error
...
func TestAntigravityInteractionsJSONResponseIncludesCreatedAtForLifecycleEnvelope(t *testing.T) {
	wantCreatedAt := time.Date(2026, time.August, 27, 1, 2, 3, 0, time.UTC).Unix()
	for _, test := range []struct {
		name string
		body string
	}{
		{
			...
			body: `{"id":"int_direct","object":"interaction","status":"completed","created":"2026-08-27T01:02:03Z","steps":[]}`,
		},
		{
			name: "nested interaction",
			body: `{"interaction":{"id":"int_nested","object":"interaction","status":"completed","created":"2026-08-27T01:02:03Z","steps":[]}}`,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			body, err := convertAntigravityInteractionJSON(io.NopCloser(strings.NewReader(test.body)), "gemini-test")
			if err != nil {
				t.Fatal(err)
			}
			...
			if !ok || createdAt != float64(wantCreatedAt) {
				t.Fatalf("created_at = %#v, want Unix seconds %d", response["created_at"], wantCreatedAt)
			}
		})
	}
}
