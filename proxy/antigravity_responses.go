...
// account's Google transport (Interactions for API keys, Cloud Code v1internal
// for OAuth) and returns one canonical Responses body/stream to its callers.
...
func executeAntigravityInteractionsRequest(ctx context.Context, account *auth.Account, model string, body []byte, stream bool, proxyURL string) (*http.Response, error) {
	...
	if stream {
		resp.Body = newAntigravityInteractionsSSEResponseBody(resp.Body, model)
		resp.Header.Set("Content-Type", "text/event-stream")
		resp.Header.Del("Content-Length")
		resp.ContentLength = -1
		return resp, nil
	}
	converted, convertErr := convertAntigravityInteractionJSON(resp.Body, model)
	if convertErr != nil {
		return nil, &Error{
			Code:       ErrorCodeUpstreamError,
			Message:    "Google Interactions returned an invalid response",
			Type:       ErrorTypeUpstreamError,
			Retryable:  true,
			HTTPStatus: http.StatusBadGateway,
			Cause:      convertErr,
...
// Google Interactions and OpenAI Responses deliberately use different output
// protocols. Keep that difference contained at the HTTP executor boundary so
// every handler can continue to consume one canonical Responses stream/body.
// Interactions server-side tool steps are intermediate execution records and
// have no lossless Responses item equivalent; they are omitted while portable
// model_output, thought summary, and client function_call steps are retained.

func convertAntigravityInteractionJSON(r io.ReadCloser, model string) ([]byte, error) {
	body, err := readBoundedAntigravityBody(r, antigravityResponseBodyLimit)
	if err != nil {
		return nil, fmt.Errorf("read Google Interactions JSON response: %w", err)
	}
	var interaction map[string]any
	if err := json.Unmarshal(body, &interaction); err != nil {
		return nil, fmt.Errorf("decode Google Interactions JSON response: %w", err)
	}
	// Accept the lifecycle envelope too. The unary create method currently
	// returns the Interaction resource directly, while this makes the boundary
	// tolerant of the same shape used by interaction.completed.
	if nested, ok := interaction["interaction"].(map[string]any); ok {
		interaction = nested
	}
	statusValue := firstAntigravityString(interaction, "status")
	if statusValue == "" {
		return nil, fmt.Errorf("Google Interactions JSON response has no status")
	}
	steps, stepsOK := interaction["steps"].([]any)
	if !stepsOK && !strings.EqualFold(statusValue, "failed") && !strings.EqualFold(statusValue, "cancelled") {
		return nil, fmt.Errorf("Google Interactions JSON response has no steps array")
	}
	output, outputText, outputErr := antigravityInteractionOutput(steps)
	if outputErr != nil {
		return nil, outputErr
	}
	status, terminalType := antigravityInteractionStatus(statusValue)
	...
	if responseModel == "" {
		responseModel = firstAntigravityString(interaction, "model")
	}
	response := map[string]any{
		"id":          "resp_ag_" + antigravityRandomHex(12),
		"object":      "response",
		"created_at":  antigravityInteractionCreatedAt(interaction),
		"status":      status,
		"model":       responseModel,
		"output":      output,
		"output_text": outputText,
	}
	if usage := antigravityInteractionsUsage(interaction["usage"]); usage != nil {
		response["usage"] = usage
	}
	switch terminalType {
	case "response.incomplete":
		response["incomplete_details"] = map[string]any{"reason": antigravityInteractionIncompleteReason(statusValue)}
	case "response.failed":
		code, message := antigravityInteractionFailure(interaction, statusValue)
		response["error"] = map[string]any{"code": code, "message": message}
	}
	return json.Marshal(response)
}

func antigravityInteractionCreatedAt(interaction map[string]any) int64 {
	createdAt := time.Now().Unix()
	if created := firstAntigravityString(interaction, "created"); created != "" {
...
func antigravityInteractionOutput(steps []any) ([]any, string, error) {
	output := make([]any, 0, len(steps))
	...
	for _, rawStep := range steps {
		...
		switch lowerStringField(step, "type") {
		case "model_output":
			text, err := antigravityInteractionTextContent(step["content"])
			if err != nil {
				return nil, "", err
			}
			item := antigravityInteractionMessageItem("msg_ag_"+antigravityRandomHex(12), text, "completed")
			output = append(output, item)
			outputText.WriteString(text)
		case "thought":
			summary, err := antigravityInteractionThoughtSummary(step["summary"])
			if err != nil {
				return nil, "", err
			}
			signature := firstAntigravityString(step, "signature")
			if summary == "" && signature == "" {
				continue
			}
			item := antigravityInteractionReasoningItem("rs_ag_"+antigravityRandomHex(12), summary, signature, "completed")
			output = append(output, item)
		case "function_call":
			name := firstAntigravityString(step, "name")
			callID := firstAntigravityString(step, "id", "call_id", "callId")
			if name == "" || callID == "" {
				return nil, "", fmt.Errorf("Google Interactions function_call is missing name or id")
			}
			arguments, err := antigravityInteractionArguments(step["arguments"])
			if err != nil {
				return nil, "", err
			}
			output = append(output, antigravityFunctionCallItem(antigravityFunctionCall{
				ItemID: "fc_ag_" + antigravityRandomHex(12), CallID: callID, Name: name, Arguments: arguments,
			}, "completed", arguments))
		default:
			// Antigravity executes search, code, browser, and other agent tools on
			// Google's side. Their call/result steps are audit trail rather than
			// client actions, so the canonical response exposes the resulting model
			// output instead of inventing incompatible OpenAI tool items.
			continue
		}
	}
	return output, outputText.String(), nil
}

func antigravityInteractionTextContent(raw any) (string, error) {
	...
	for _, rawPart := range parts {
		...
		partType := lowerStringField(part, "type")
		if partType != "" && partType != "text" {
			return "", fmt.Errorf("Google Interactions model_output contains unsupported %s content", partType)
		}
...
func antigravityInteractionThoughtSummary(raw any) (string, error) {
	if text, ok := raw.(string); ok {
		return text, nil
	}
	parts, ok := raw.([]any)
	...
	for _, rawPart := range parts {
		...
		partType := lowerStringField(part, "type")
		if partType != "" && partType != "text" {
			// Image thought summaries cannot be represented by a Responses
			// summary_text item without changing their meaning.
			return "", fmt.Errorf("Google Interactions thought contains unsupported %s summary", partType)
		}
		if value, ok := part["text"].(string); ok {
			text.WriteString(value)
		}
...
func antigravityInteractionArguments(raw any) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("encode Google Interactions function arguments: %w", err)
	}
	return string(encoded), nil
}

func antigravityInteractionMessageItem(id, text, status string) map[string]any {
...
func antigravityInteractionReasoningItem(id, summary, signature, status string) map[string]any {
	item := map[string]any{"id": id, "type": "reasoning", "status": status, "summary": []any{}}
	if summary != "" {
...
func antigravityInteractionStatus(status string) (string, string) {
	switch strings.ToLower(strings.TrimSpace(status)) {
	case "", "completed", "requires_action":
		// Responses represents a client function call as a completed response
		// containing a function_call item, not as a requires_action status.
...
func antigravityInteractionIncompleteReason(status string) string {
	if strings.EqualFold(strings.TrimSpace(status), "budget_exceeded") {
		return "max_output_tokens"
	}
	return "max_output_tokens"
}

func antigravityInteractionFailure(interaction map[string]any, status string) (string, string) {
	code := "antigravity_interaction_" + strings.ToLower(strings.TrimSpace(status))
	if strings.HasSuffix(code, "_") {
		code += "failed"
	}
	message := "Google Interactions ended with status " + strings.TrimSpace(status)
	if errorValue, ok := interaction["error"].(map[string]any); ok {
		if value := firstAntigravityString(errorValue, "code", "type"); value != "" {
			code = value
		}
		if value := firstAntigravityString(errorValue, "message"); value != "" {
			message = value
		}
	}
	if strings.TrimSpace(status) == "" && message == "Google Interactions ended with status " {
		message = "Google Interactions failed"
	}
	return code, message
}

func antigravityInteractionsUsage(raw any) map[string]any {
	usage, _ := raw.(map[string]any)
	if usage == nil {
		return nil
	}
	inputTokens := antigravityFirstUsageTokenCount(usage, "total_input_tokens", "input_tokens", "prompt_tokens")
	generatedTokens := antigravityFirstUsageTokenCount(usage, "total_output_tokens", "output_tokens", "completion_tokens")
	reasoningTokens := antigravityFirstUsageTokenCount(usage, "total_thought_tokens", "reasoning_tokens")
	outputTokens := generatedTokens + reasoningTokens
	totalTokens := antigravityFirstUsageTokenCount(usage, "total_tokens")
	...
	if cached := antigravityFirstUsageTokenCount(usage, "total_cached_tokens", "cached_tokens"); cached > 0 {
...
func antigravityFirstUsageTokenCount(usage map[string]any, keys ...string) int64 {
	for _, key := range keys {
		if value, exists := usage[key]; exists {
			return antigravityUsageTokenCount(value)
		}
	}
	return 0
}

type antigravityInteractionStreamStep struct {
	index       int
	kind        string
	outputIndex int
	itemID      string
...
type antigravityInteractionsSSEBody struct {
	...
	started    bool
	terminal   bool
	steps      map[int]*antigravityInteractionStreamStep
	order      []*antigravityInteractionStreamStep
	usage      map[string]any
	outputText strings.Builder
}

func newAntigravityInteractionsSSEResponseBody(source io.ReadCloser, model string) io.ReadCloser {
	return &antigravityInteractionsSSEBody{
		source: source, reader: bufio.NewReader(source),
		responseID: "resp_ag_" + antigravityRandomHex(12),
		model:      strings.TrimSpace(model), createdAt: time.Now().Unix(),
		steps: make(map[int]*antigravityInteractionStreamStep),
	}
}

func (b *antigravityInteractionsSSEBody) Close() error {
...
func (b *antigravityInteractionsSSEBody) enqueue(eventType string, fields map[string]any) {
	if fields == nil {
...
func (b *antigravityInteractionsSSEBody) response(status string, output []any) map[string]any {
	if output == nil {
		output = []any{}
	}
	return map[string]any{
...
func (b *antigravityInteractionsSSEBody) enqueueStart(interaction map[string]any) {
	if b.started {
		return
	}
	if b.model == "" && interaction != nil {
		b.model = firstAntigravityString(interaction, "model")
	}
	if interaction != nil {
		b.createdAt = antigravityInteractionCreatedAt(interaction)
	}
	b.started = true
...
func (b *antigravityInteractionsSSEBody) enqueueFailure(code, message string, statusCode ...int) {
	if b.terminal {
		return
	}
	b.enqueueStart(nil)
...
func antigravityInteractionPortableStepKind(kind string) bool {
...
func (b *antigravityInteractionsSSEBody) startStep(index int, raw map[string]any) {
	if b.terminal || raw == nil {
		return
	}
	kind := lowerStringField(raw, "type")
	if !antigravityInteractionPortableStepKind(kind) {
		return
	}
	step := b.steps[index]
	if step == nil {
		step = &antigravityInteractionStreamStep{
			index: index, kind: kind, outputIndex: len(b.order),
		}
		switch kind {
		case "model_output":
			step.itemID = "msg_ag_" + antigravityRandomHex(12)
		case "thought":
			step.itemID = "rs_ag_" + antigravityRandomHex(12)
		case "function_call":
			step.itemID = "fc_ag_" + antigravityRandomHex(12)
			step.callID = firstAntigravityString(raw, "id", "call_id", "callId")
			step.name = firstAntigravityString(raw, "name")
			if step.callID == "" || step.name == "" {
				b.enqueueFailure(ErrorCodeUpstreamError, "Google Interactions function_call is missing name or id")
				return
			}
		}
		b.steps[index] = step
		b.order = append(b.order, step)
		b.enqueueStepAdded(step)
	}
	switch kind {
	case "model_output":
		text, err := antigravityInteractionTextContent(raw["content"])
		if err != nil {
			b.enqueueFailure("unsupported_output", err.Error(), http.StatusBadRequest)
			return
		}
		b.appendStepText(step, text)
	case "thought":
		summary, err := antigravityInteractionThoughtSummary(raw["summary"])
		if err != nil {
			b.enqueueFailure("unsupported_output", err.Error(), http.StatusBadRequest)
			return
		}
		b.appendThoughtSummary(step, summary)
		if signature := firstAntigravityString(raw, "signature"); signature != "" {
			b.appendThoughtSignature(step, signature)
		}
	case "function_call":
		if arguments, ok := raw["arguments"].(map[string]any); ok && len(arguments) > 0 {
			if encoded, err := antigravityInteractionArguments(arguments); err == nil {
				b.appendFunctionArguments(step, encoded)
			}
		} else if arguments, ok := raw["arguments"].(string); ok && strings.TrimSpace(arguments) != "" {
			b.appendFunctionArguments(step, arguments)
		}
	}
}

func (b *antigravityInteractionsSSEBody) enqueueStepAdded(step *antigravityInteractionStreamStep) {
	if step == nil {
		return
	}
	switch step.kind {
	case "model_output":
		b.enqueue("response.output_item.added", map[string]any{
			"output_index": step.outputIndex,
			"item":         map[string]any{"id": step.itemID, "type": "message", "status": "in_progress", "role": "assistant", "content": []any{}},
		})
		b.enqueue("response.content_part.added", map[string]any{
			"item_id": step.itemID, "output_index": step.outputIndex, "content_index": 0,
			"part": map[string]any{"type": "output_text", "text": "", "annotations": []any{}},
		})
	case "thought":
		b.enqueue("response.output_item.added", map[string]any{
			"output_index": step.outputIndex,
			"item":         antigravityInteractionReasoningItem(step.itemID, "", "", "in_progress"),
		})
	case "function_call":
		b.enqueue("response.output_item.added", map[string]any{
			"output_index": step.outputIndex,
			"item": antigravityFunctionCallItem(antigravityFunctionCall{
				ItemID: step.itemID, CallID: step.callID, Name: step.name,
			}, "in_progress", ""),
		})
	}
}

func (b *antigravityInteractionsSSEBody) appendStepText(step *antigravityInteractionStreamStep, text string) {
	if step == nil || text == "" || step.stopped || b.terminal {
		return
	}
	step.text.WriteString(text)
	b.outputText.WriteString(text)
	b.enqueue("response.output_text.delta", map[string]any{
		"item_id": step.itemID, "output_index": step.outputIndex, "content_index": 0, "delta": text,
	})
}

func (b *antigravityInteractionsSSEBody) appendThoughtSummary(step *antigravityInteractionStreamStep, text string) {
	if step == nil || text == "" || step.stopped || b.terminal {
		return
	}
	step.text.WriteString(text)
	b.enqueue("response.reasoning_summary_text.delta", map[string]any{
		"item_id": step.itemID, "output_index": step.outputIndex, "summary_index": 0, "delta": text,
	})
}

func (b *antigravityInteractionsSSEBody) appendThoughtSignature(step *antigravityInteractionStreamStep, signature string) {
	if step == nil || signature == "" || step.stopped || b.terminal {
		return
	}
	step.signature.WriteString(signature)
	b.enqueue("response.reasoning.encrypted_content.delta", map[string]any{
		"item_id": step.itemID, "output_index": step.outputIndex, "delta": signature,
	})
}

func (b *antigravityInteractionsSSEBody) appendFunctionArguments(step *antigravityInteractionStreamStep, arguments string) {
	if step == nil || arguments == "" || step.stopped || b.terminal {
		return
	}
	step.arguments.WriteString(arguments)
	b.enqueue("response.function_call_arguments.delta", map[string]any{
		"item_id": step.itemID, "call_id": step.callID, "output_index": step.outputIndex, "delta": arguments,
	})
}

func (b *antigravityInteractionsSSEBody) applyDelta(index int, delta map[string]any) {
	if b.terminal || delta == nil {
		return
	}
	deltaType := lowerStringField(delta, "type")
	step := b.steps[index]
	if step == nil {
		// Text/thought deltas can still be projected when an intermediary strips
		// step.start. Function deltas cannot: name and call id are required.
		switch deltaType {
		case "text":
			b.startStep(index, map[string]any{"type": "model_output"})
		case "thought", "thought_summary":
			b.startStep(index, map[string]any{"type": "thought"})
		default:
			return
		}
		step = b.steps[index]
	...
	case "text":
		...
		if step.kind == "thought" {
			b.appendThoughtSummary(step, text)
		} else if step.kind == "model_output" {
			b.appendStepText(step, text)
		}
	case "thought", "thought_summary":
		content, _ := delta["content"].(map[string]any)
		text := firstAntigravityString(content, "text")
		if text == "" {
			text = firstAntigravityString(delta, "text")
		}
		b.appendThoughtSummary(step, text)
	case "thought_signature":
		b.appendThoughtSignature(step, firstAntigravityString(delta, "signature"))
	case "arguments", "arguments_delta":
		arguments := firstAntigravityString(delta, "arguments", "partial_arguments")
		if step.kind == "function_call" {
			b.appendFunctionArguments(step, arguments)
		}
	case "image", "audio":
		if step.kind == "model_output" {
			b.enqueueFailure("unsupported_output", "Google Interactions returned unsupported "+deltaType+" content", http.StatusBadRequest)
		}
	default:
		// Provider-side tool progress remains an omitted intermediate step.
	}
}

func (b *antigravityInteractionsSSEBody) stopStep(index int) {
	step := b.steps[index]
	if step == nil || step.stopped || b.terminal {
		return
	}
	step.stopped = true
	switch step.kind {
	case "model_output":
		...
		item := antigravityInteractionMessageItem(step.itemID, text, "completed")
		b.enqueue("response.output_text.done", map[string]any{
			"item_id": step.itemID, "output_index": step.outputIndex, "content_index": 0, "text": text,
		})
		b.enqueue("response.content_part.done", map[string]any{
			"item_id": step.itemID, "output_index": step.outputIndex, "content_index": 0, "part": content,
		})
		b.enqueue("response.output_item.done", map[string]any{"output_index": step.outputIndex, "item": item})
	case "thought":
		if step.text.Len() > 0 {
			b.enqueue("response.reasoning_summary_text.done", map[string]any{
				"item_id": step.itemID, "output_index": step.outputIndex, "summary_index": 0, "text": step.text.String(),
			})
		}
		if step.signature.Len() > 0 {
			b.enqueue("response.reasoning.encrypted_content.done", map[string]any{
				"item_id": step.itemID, "output_index": step.outputIndex, "encrypted_content": step.signature.String(),
			})
		}
		item := antigravityInteractionReasoningItem(step.itemID, step.text.String(), step.signature.String(), "completed")
		b.enqueue("response.output_item.done", map[string]any{"output_index": step.outputIndex, "item": item})
	case "function_call":
		...
		b.enqueue("response.function_call_arguments.done", map[string]any{
			"item_id": step.itemID, "call_id": step.callID, "output_index": step.outputIndex, "arguments": arguments,
		})
		item := antigravityFunctionCallItem(antigravityFunctionCall{
			ItemID: step.itemID, CallID: step.callID, Name: step.name, Arguments: arguments,
		}, "completed", arguments)
		b.enqueue("response.output_item.done", map[string]any{"output_index": step.outputIndex, "item": item})
	}
}

func (b *antigravityInteractionsSSEBody) finalOutput() []any {
	output := make([]any, 0, len(b.order))
	for _, step := range b.order {
		if step == nil {
			continue
		}
		if !step.stopped {
			b.stopStep(step.index)
		}
		switch step.kind {
		case "model_output":
			output = append(output, antigravityInteractionMessageItem(step.itemID, step.text.String(), "completed"))
		case "thought":
			if step.text.Len() > 0 || step.signature.Len() > 0 {
				output = append(output, antigravityInteractionReasoningItem(step.itemID, step.text.String(), step.signature.String(), "completed"))
			}
		case "function_call":
			arguments := step.arguments.String()
			if arguments == "" {
...
func (b *antigravityInteractionsSSEBody) enqueueTerminal(interaction map[string]any) {
	if b.terminal {
		return
	}
	b.enqueueStart(interaction)
	statusValue := firstAntigravityString(interaction, "status")
	status, eventType := antigravityInteractionStatus(statusValue)
	if usage := antigravityInteractionsUsage(interaction["usage"]); usage != nil {
		b.usage = usage
	}
	...
	output := b.finalOutput()
	b.terminal = true
	response := b.response(status, output)
	response["output_text"] = b.outputText.String()
	if b.usage != nil {
		response["usage"] = b.usage
	}
...
func (b *antigravityInteractionsSSEBody) handleEvent(event map[string]any) {
	eventType := lowerStringField(event, "event_type")
	switch eventType {
	case "interaction.created":
		interaction, _ := event["interaction"].(map[string]any)
		b.enqueueStart(interaction)
	case "step.start":
		b.enqueueStart(nil)
		step, _ := event["step"].(map[string]any)
		b.startStep(int(antigravityUsageTokenCount(event["index"])), step)
	case "step.delta":
		b.enqueueStart(nil)
		delta, _ := event["delta"].(map[string]any)
		b.applyDelta(int(antigravityUsageTokenCount(event["index"])), delta)
	case "step.stop":
		b.enqueueStart(nil)
		if usage := antigravityInteractionsUsage(event["usage"]); usage != nil {
			b.usage = usage
		}
		b.stopStep(int(antigravityUsageTokenCount(event["index"])))
	case "interaction.completed", "interaction.incomplete", "interaction.failed", "interaction.cancelled":
		interaction, _ := event["interaction"].(map[string]any)
		if interaction == nil {
			interaction = map[string]any{}
		}
		if _, exists := interaction["status"]; !exists {
			interaction["status"] = strings.TrimPrefix(eventType, "interaction.")
		}
		b.enqueueTerminal(interaction)
	case "error":
		errorValue, _ := event["error"].(map[string]any)
		code := firstAntigravityString(errorValue, "code", "type")
		if code == "" {
			code = ErrorCodeUpstreamError
		}
		message := firstAntigravityString(errorValue, "message")
		if message == "" {
			message = "Google Interactions stream failed"
		}
		b.enqueueFailure(code, message)
	default:
		// interaction.in_progress/requires_action are lifecycle notifications;
		// the authoritative terminal and usage arrive in interaction.completed.
	}
}

func (b *antigravityInteractionsSSEBody) Read(p []byte) (int, error) {
	for b.queue.Len() == 0 {
		if b.terminal {
			return 0, io.EOF
		}
		...
		if err != nil {
			b.enqueueFailure(ErrorCodeUpstreamStreamBreak, "Google Interactions stream ended before interaction.completed")
			continue
		}
		if len(data) == 0 {
			continue
		...
		if err := json.Unmarshal(data, &event); err != nil {
			b.enqueueFailure(ErrorCodeUpstreamError, "Google Interactions emitted invalid SSE JSON")
			continue
		}
		b.handleEvent(event)
	}
