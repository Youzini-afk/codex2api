...
// SchedulerOutboxReplayWatermark returns the newest currently visible event
// strictly older than replaySince. A consumer starts after this id, so every
// event in the replay window is applied again. This is intentional: PostgreSQL
// BIGSERIAL ids are allocated at INSERT, while row visibility waits for COMMIT;
// a transaction that commits out of order can therefore surface below a
// concurrently observed high watermark.
//
// The caller must size the replay window to cover its longest real write
// transaction. A transaction that remains open longer than that window can
// still commit below the returned watermark and needs a separate reconcile.
func (db *DB) SchedulerOutboxReplayWatermark(ctx context.Context, replaySince time.Time) (int64, error) {
	if db == nil || db.conn == nil || replaySince.IsZero() {
		return 0, nil
	}
	var watermark int64
	err := db.conn.QueryRowContext(ctx, `SELECT COALESCE(MAX(id),0) FROM scheduler_outbox WHERE created_at<$1`, db.timeArg(replaySince)).Scan(&watermark)
	return watermark, err
}
...
// that can change routing eligibility emit an event. Antigravity quota and
// permission documents are likewise reduced to their durable fence bits, so a
// remaining-quota or observation-timestamp refresh does not fan out a reload.
	...
	const query = `
		...
		END;
			...
			WHEN OLD.proxy_url IS NOT NEW.proxy_url OR OLD.status IS NOT NEW.status OR OLD.error_message IS NOT NEW.error_message
			  ...
			  OR OLD.usage_reserve_percent_5h IS NOT NEW.usage_reserve_percent_5h OR OLD.usage_reserve_percent_7d IS NOT NEW.usage_reserve_percent_7d
		  ...
		  OR COALESCE(json_extract(OLD.credentials,'$.allowed_api_key_ids'),'') IS NOT COALESCE(json_extract(NEW.credentials,'$.allowed_api_key_ids'),'')
			  ...
			  OR (
				CASE
				  WHEN json_valid(COALESCE(json_extract(OLD.credentials,'$.antigravity_quota'),''))
				   AND json_type(json_extract(OLD.credentials,'$.antigravity_quota'),'$.forbidden')='true' THEN 1
				  ELSE 0
				END
			  ) IS NOT (
				CASE
				  WHEN json_valid(COALESCE(json_extract(NEW.credentials,'$.antigravity_quota'),''))
				   AND json_type(json_extract(NEW.credentials,'$.antigravity_quota'),'$.forbidden')='true' THEN 1
				  ELSE 0
				END
			  )
			  OR (
				CASE
				  WHEN json_valid(COALESCE(NULLIF(TRIM(json_extract(OLD.credentials,'$.antigravity_permissions')),''),json_extract(OLD.credentials,'$.antigravity_entitlements'),''))
				   AND (json_type(COALESCE(NULLIF(TRIM(json_extract(OLD.credentials,'$.antigravity_permissions')),''),json_extract(OLD.credentials,'$.antigravity_entitlements'),''),'$.allowed') IS NULL
				     OR json_type(COALESCE(NULLIF(TRIM(json_extract(OLD.credentials,'$.antigravity_permissions')),''),json_extract(OLD.credentials,'$.antigravity_entitlements'),''),'$.allowed')='false')
				   AND (TRIM(COALESCE(json_extract(COALESCE(NULLIF(TRIM(json_extract(OLD.credentials,'$.antigravity_permissions')),''),json_extract(OLD.credentials,'$.antigravity_entitlements'),''),'$.reason'),''))<>''
				     OR TRIM(COALESCE(json_extract(COALESCE(NULLIF(TRIM(json_extract(OLD.credentials,'$.antigravity_permissions')),''),json_extract(OLD.credentials,'$.antigravity_entitlements'),''),'$.updated_at'),''))<>'') THEN 1
				  ELSE 0
				END
			  ) IS NOT (
				CASE
				  WHEN json_valid(COALESCE(NULLIF(TRIM(json_extract(NEW.credentials,'$.antigravity_permissions')),''),json_extract(NEW.credentials,'$.antigravity_entitlements'),''))
				   AND (json_type(COALESCE(NULLIF(TRIM(json_extract(NEW.credentials,'$.antigravity_permissions')),''),json_extract(NEW.credentials,'$.antigravity_entitlements'),''),'$.allowed') IS NULL
				     OR json_type(COALESCE(NULLIF(TRIM(json_extract(NEW.credentials,'$.antigravity_permissions')),''),json_extract(NEW.credentials,'$.antigravity_entitlements'),''),'$.allowed')='false')
				   AND (TRIM(COALESCE(json_extract(COALESCE(NULLIF(TRIM(json_extract(NEW.credentials,'$.antigravity_permissions')),''),json_extract(NEW.credentials,'$.antigravity_entitlements'),''),'$.reason'),''))<>''
				     OR TRIM(COALESCE(json_extract(COALESCE(NULLIF(TRIM(json_extract(NEW.credentials,'$.antigravity_permissions')),''),json_extract(NEW.credentials,'$.antigravity_entitlements'),''),'$.updated_at'),''))<>'') THEN 1
				  ELSE 0
				END
			  )
			  OR (
				CASE
				  WHEN TRIM(COALESCE(json_extract(OLD.credentials,'$.antigravity_sync_error'),''))<>'' AND (
				    TRIM(COALESCE(json_extract(OLD.credentials,'$.antigravity_permanent_refresh_error'),''))=TRIM(COALESCE(json_extract(OLD.credentials,'$.antigravity_sync_error'),''))
				    OR INSTR(LOWER(TRIM(COALESCE(json_extract(OLD.credentials,'$.antigravity_sync_error'),''))),'changed google principal')>0
				    OR SUBSTR(TRIM(COALESCE(json_extract(OLD.credentials,'$.antigravity_sync_error'),'')),1,LENGTH('Antigravity credential rotated before Google identity could be reverified'))='Antigravity credential rotated before Google identity could be reverified'
				  ) THEN 1 ELSE 0
				END
			  ) IS NOT (
				CASE
				  WHEN TRIM(COALESCE(json_extract(NEW.credentials,'$.antigravity_sync_error'),''))<>'' AND (
				    TRIM(COALESCE(json_extract(NEW.credentials,'$.antigravity_permanent_refresh_error'),''))=TRIM(COALESCE(json_extract(NEW.credentials,'$.antigravity_sync_error'),''))
				    OR INSTR(LOWER(TRIM(COALESCE(json_extract(NEW.credentials,'$.antigravity_sync_error'),''))),'changed google principal')>0
				    OR SUBSTR(TRIM(COALESCE(json_extract(NEW.credentials,'$.antigravity_sync_error'),'')),1,LENGTH('Antigravity credential rotated before Google identity could be reverified'))='Antigravity credential rotated before Google identity could be reverified'
				  ) THEN 1 ELSE 0
				END
			  )
...
func (db *DB) installPostgresSchedulerOutboxTriggers(ctx context.Context) error {
	const query = `
			CREATE OR REPLACE FUNCTION codex2api_antigravity_scheduler_fences(credentials JSONB) RETURNS TEXT AS $$
			DECLARE
				quota_raw TEXT;
				permissions_raw TEXT;
				parsed JSONB;
				quota_forbidden BOOLEAN := false;
				permission_denied BOOLEAN := false;
				sync_fenced BOOLEAN := false;
				sync_error TEXT;
				permanent_refresh_error TEXT;
			BEGIN
				quota_raw := COALESCE(credentials->>'antigravity_quota', '');
				IF btrim(quota_raw) <> '' THEN
					BEGIN
						parsed := quota_raw::jsonb;
						quota_forbidden := jsonb_typeof(parsed->'forbidden') = 'boolean' AND COALESCE((parsed->>'forbidden')::boolean, false);
					EXCEPTION WHEN OTHERS THEN
						quota_forbidden := false;
					END;
				END IF;

				permissions_raw := COALESCE(NULLIF(btrim(credentials->>'antigravity_permissions'), ''), credentials->>'antigravity_entitlements', '');
				IF btrim(permissions_raw) <> '' THEN
					BEGIN
						parsed := permissions_raw::jsonb;
						permission_denied := (NOT (parsed ? 'allowed') OR (jsonb_typeof(parsed->'allowed') = 'boolean'
							AND NOT COALESCE((parsed->>'allowed')::boolean, false)))
							AND (btrim(COALESCE(parsed->>'reason', '')) <> '' OR btrim(COALESCE(parsed->>'updated_at', '')) <> '');
					EXCEPTION WHEN OTHERS THEN
						permission_denied := false;
					END;
				END IF;

				sync_error := btrim(COALESCE(credentials->>'antigravity_sync_error', ''));
				permanent_refresh_error := btrim(COALESCE(credentials->>'antigravity_permanent_refresh_error', ''));
				sync_fenced := sync_error <> '' AND (
					(permanent_refresh_error <> '' AND permanent_refresh_error = sync_error) OR
					position('changed google principal' in lower(sync_error)) > 0 OR
					left(sync_error, length('Antigravity credential rotated before Google identity could be reverified')) = 'Antigravity credential rotated before Google identity could be reverified'
				);
				RETURN concat(quota_forbidden, ':', permission_denied, ':', sync_fenced);
			END;
		...
		CREATE TRIGGER scheduler_outbox_accounts_insert AFTER INSERT ON accounts FOR EACH ROW EXECUTE FUNCTION codex2api_scheduler_outbox_row('account','id');
			...
			OLD.cooldown_reason IS DISTINCT FROM NEW.cooldown_reason OR OLD.cooldown_until IS DISTINCT FROM NEW.cooldown_until OR
				...
				OLD.usage_reserve_percent_5h IS DISTINCT FROM NEW.usage_reserve_percent_5h OR OLD.usage_reserve_percent_7d IS DISTINCT FROM NEW.usage_reserve_percent_7d OR
			...
			COALESCE(OLD.credentials->'custom_headers','null'::jsonb) IS DISTINCT FROM COALESCE(NEW.credentials->'custom_headers','null'::jsonb) OR
				...
				codex2api_antigravity_scheduler_fences(OLD.credentials) IS DISTINCT FROM codex2api_antigravity_scheduler_fences(NEW.credentials) OR
