...
func NewWithOptions(driver string, dsn string, options Options) (*DB, error) {
	...
	// 所有同步初始化、可选数据迁移和用量汇总均完成后，才允许后台 writer 启动。
	db.startLogFlusher()
...
func (db *DB) migratePostgresSchema(ctx context.Context) error {
	...
	-- 官方每日用量是上游短期窗口的持久快照，旧日期无法重新抓取；它不是
	-- usage_logs 的可重建汇总，因此属于 SQLite→PostgreSQL 迁移的业务数据。
	...
	-- 只在这里建空表，不插入默认 singleton；自动导入的空目标检查必须保持
	-- 无业务行，源 SQLite 若已有用户配置则由导入事务原样复制。
...
func (db *DB) UpdateSystemSettings(ctx context.Context, s *SystemSettings) error {
	...
	_, err := db.conn.ExecContext(ctx, `
			INSERT INTO system_settings (
				...
				public_image_studio_page_enabled,
					...
					codex_ws_silent_retry_enabled,
						...
						codex_reasoning_effort_alias_enabled,
		...
		s.CodexFastModelAliasEnabled, s.CodexReasoningEffortAliasEnabled, s.CodexFastTierInterceptEnabled,
...
func (db *DB) ListActiveByChannel(ctx context.Context, channel string) ([]*AccountRow, error) {
	...
	query := `
		SELECT id, name, platform, type, credentials, proxy_url, status, cooldown_reason, cooldown_until, error_message,
		       COALESCE(enabled, true), COALESCE(locked, false), COALESCE(credit_enabled, false),
		       COALESCE(credit_skip_usage_window, false), COALESCE(skip_warm_tier, false),
		       score_bias_override, base_concurrency_override, usage_reserve_percent_5h, usage_reserve_percent_7d,
...
func (db *DB) UpdateAccountSchedulerConfig(ctx context.Context, id int64, scoreBiasOverride interface{}, baseConcurrencyOverride interface{}, usageReserve5h OptionalNullInt64, usageReserve7d OptionalNullInt64, allowedAPIKeyIDs OptionalInt64Slice) error {
	scoreBias := optionalNullInt64FromAny(scoreBiasOverride)
	baseConcurrency := optionalNullInt64FromAny(baseConcurrencyOverride)
	tx, err := db.conn.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if scoreBias.Set || baseConcurrency.Set || usageReserve5h.Set || usageReserve7d.Set {
		...
		if scoreBias.Set {
			add("score_bias_override", nullableInt64Value(scoreBias.Value))
		}
		if baseConcurrency.Set {
			add("base_concurrency_override", nullableInt64Value(baseConcurrency.Value))
		}
		if usageReserve5h.Set {
			add("usage_reserve_percent_5h", nullableInt64Value(usageReserve5h.Value))
		}
		if usageReserve7d.Set {
			add("usage_reserve_percent_7d", nullableInt64Value(usageReserve7d.Value))
		}
...
func optionalNullInt64FromAny(value interface{}) OptionalNullInt64 {
	switch v := value.(type) {
	case OptionalNullInt64:
		return v
	case sql.NullInt64:
		return OptionalNullInt64{Set: true, Value: v}
	default:
		return OptionalNullInt64{}
	}
}
...
func (db *DB) UpdateAccountSchedulerMetadata(ctx context.Context, id int64, scoreBiasOverride OptionalNullInt64, baseConcurrencyOverride OptionalNullInt64, usageReserve5h OptionalNullInt64, usageReserve7d OptionalNullInt64, skipWarmTier OptionalBool, allowedAPIKeyIDs OptionalInt64Slice, tags OptionalStringSlice, groupIDs OptionalInt64Slice, proxyURL OptionalString, credentialUpdates map[string]interface{}) error {
	return db.withSQLiteWriteLock(ctx, func() error {
		tx, err := db.conn.BeginTx(ctx, nil)
		if err != nil {
			return err
...
func (db *DB) UpdateUsageSnapshotFull(ctx context.Context, id int64, pct7d float64, reset7dAt time.Time, pct5h float64, reset5hAt time.Time, updatedAt7d time.Time, updatedAt5h time.Time) error {
	fields := map[string]interface{}{
		...
		"codex_7d_usage_updated_at": updatedAt7d.Format(time.RFC3339),
...
func (db *DB) ListDeleted(ctx context.Context) ([]*AccountRow, error) {
	query := `
		SELECT id, name, platform, type, credentials, proxy_url, status, cooldown_reason, cooldown_until, error_message,
		       ...
		       COALESCE(credit_skip_usage_window, false), COALESCE(skip_warm_tier, false),
		       score_bias_override, base_concurrency_override, usage_reserve_percent_5h, usage_reserve_percent_7d,
		       COALESCE(tags, '[]'), COALESCE(note, ''), created_at, updated_at, deleted_at,
		       COALESCE(credential_generation, 1), COALESCE(credential_family_id, '')
		FROM accounts
...
// InsertPendingAccountWithCredentials atomically inserts a disabled account
// together with its review metadata, so a partially completed self-service
// submission can never become schedulable after a restart.
func (db *DB) InsertPendingAccountWithCredentials(ctx context.Context, name string, credentials map[string]interface{}, proxyURL, note string, tags []string) (int64, error) {
	if credentials == nil {
		credentials = map[string]interface{}{}
	}
	...
	return db.insertRowID(ctx,
		`INSERT INTO accounts (name, credentials, proxy_url, enabled, note, tags) VALUES ($1, $2, $3, false, $4, $5::jsonb) RETURNING id`,
		`INSERT INTO accounts (name, credentials, proxy_url, enabled, note, tags) VALUES (?, ?, ?, 0, ?, ?)`,
		name, credJSON, proxyURL, note, tagsJSON,
	)
}
