...
func (db *DB) ListActiveByIDs(ctx context.Context, ids []int64) ([]*AccountRow, error) {
	...
	query := `SELECT id, name, platform, type, credentials, proxy_url, status, cooldown_reason,
		...
		usage_reserve_percent_5h, usage_reserve_percent_7d,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&row.BaseConcurrencyOverride, &row.UsageReservePercent5h, &row.UsageReservePercent7d,
			&tagsRaw, &row.Note, &createdRaw, &updatedRaw,
